...
public final class PaymentIntent implements Parcelable {
    ...
    public final static class Output implements Parcelable {
        ...
        public Output(final Coin amount, final Script script, final byte[] vReserve) {
            this.amount = amount;
            this.script = script;
    ...
    public PaymentIntent(@Nullable final Standard standard, @Nullable final String payeeName, @Nullable final String payeeVerifiedBy,
                         @Nullable final Output[] outputs, @Nullable final String memo, @Nullable final String paymentUrl, @Nullable final byte[] payeeData,
                         @Nullable final String paymentRequestUrl, @Nullable final byte[] paymentRequestHash, boolean useInstantX, String assetName) {
        this(standard, payeeName, payeeVerifiedBy, outputs, memo, paymentUrl, payeeData, paymentRequestUrl, paymentRequestHash);
        this.useInstantX = useInstantX;
    ...
    public static PaymentIntent fromBitcoinUri(final BitcoinURI bitcoinUri) {
        ...
        final String assetName = bitcoinUri.getAssetName();
        int exponent = Coin.SMALLEST_UNIT_EXPONENT;
        if(assetName != null && !assetName.equalsIgnoreCase(SafeConstant.SAFE_FLAG)){
            BaseDaoImpl assetDao = new BaseDaoImpl(IssueData.class);
            try {
                IssueData issue = (IssueData) assetDao.queryForFirst("assetName", assetName);
                if(issue != null){
                    exponent = (int)issue.decimals;
                }
            } catch (SQLException e) {
            }
        }
        BigDecimal realDecimal;
        if (!TextUtils.isEmpty(bitcoinUri.getAmount())&&!bitcoinUri.getAmount().equals(".")) {
            realDecimal = new BigDecimal(bitcoinUri.getAmount()).multiply(new BigDecimal(SafeUtils.getRealDecimals(exponent)));
        } else {
            realDecimal = new BigDecimal(0);
        }
        if (realDecimal.compareTo(new BigDecimal(Long.MAX_VALUE)) >= 0) {
            realDecimal = new BigDecimal(0);
        }
        final Output[] outputs = address != null ? buildSimplePayTo(Coin.valueOf(realDecimal.longValue()), address) : null;
        ...
        log.info("--------fromBitcoinUri assetName = {}", assetName);
        return new PaymentIntent(PaymentIntent.Standard.BIP21, null, null, outputs, bitcoinUri.getLabel(),
                ...
                paymentRequestHash, useInstantSend, assetName);
    }
    ...
    public PaymentIntent getPaymentIntentOutput(Output[] outputs){
    ...
    // TODO: 2018/3/20 zhangmiao update
    public SendRequest toSendRequest(long nUnlockHeight) {
        ...
        for (final PaymentIntent.Output output : outputs) {
            if (output.vReserve != null) {
                transaction.addOutput(output.amount, output.script, nUnlockHeight, output.vReserve);
            } else {
                transaction.addOutput(output.amount, output.script, nUnlockHeight, SafeConstant.SAFE_FLAG.getBytes());
            }
        }
        if (SafeConstant.getLastBlockHeight() >= Constants.NETWORK_PARAMETERS.getStartSposHeight()){
            transaction.setVersion(SafeConstant.SPOS_TX_VERSION);
        } else {
            transaction.setVersion(SafeConstant.APP_TX_VERSION);
        }
    ...
    public static Output buildOutPut(final Coin amount, final Address address) {
        return new Output(amount, ScriptBuilder.createOutputScript(address));
    }

    public static Output buildOutPut(final Coin amount, final Address address,  byte[] vReserve) {
        return new Output(amount, ScriptBuilder.createOutputScript(address), vReserve);
    }
