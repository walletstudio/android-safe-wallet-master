...
public class Configuration {
    ...
    public static final String PREFS_KEY_MUST_DOWNLOAD_BLOCK = "must_download_block";
    ...
    public MonetaryFormat getFormat() {
        final int shift = getBtcShift();
        int precision = getBtcPrecision();
        return new MonetaryFormat().shift(shift).minDecimals(precision).optionalDecimals();
    }
    ...
    public Uri getBlockExplorer() {
        return Uri.parse(prefs.getString(PREFS_KEY_BLOCK_EXPLORER, Constants.getBlockExplorer()));
    }
    ...
    public boolean remindBackup() {
        return prefs.getBoolean(PREFS_KEY_REMIND_BACKUP, false);
    }
    ...
    public int getLastVersionCode() {
        return prefs.getInt(PREFS_KEY_LAST_VERSION, 0);
    }
    ...
    public void setMustDownloadBlock(boolean isMust){
        prefs.edit().putBoolean(PREFS_KEY_MUST_DOWNLOAD_BLOCK, isMust).apply();
    }

    public boolean getMustDownloadBlock(){
        return prefs.getBoolean(PREFS_KEY_MUST_DOWNLOAD_BLOCK, false);
    }
