...
public class SafeUtils {
     ...
     * @return
     */
    public static boolean validSposReserve(TransactionOutput output, byte[] vReserve){
        int cursor = 0;
        int length = 8;
        ...
        cursor += length;
        length = 2;
        long sposVersion = Utils.readUint16(vReserve, cursor);
        cursor += length;
        length = 20;
...
//        log.info("-------keyId = {}, signData = {}, output = {}", Utils.HEX.encode(keyId), Utils.HEX.encode(signData), Utils.HEX.encode(output.bitcoinSerialize()));
//        log.info("-------pubkey = {}, ecKey = {}", Utils.HEX.encode(ecKey.getPubKeyHash()), ecKey);
