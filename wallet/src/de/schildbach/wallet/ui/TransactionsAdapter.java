...
public class TransactionsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void replace(final Transaction tx, final String assetId, long decimals) {
        transactions.clear();
        transactions.add(tx);
        ...
        private void bind(final Transaction tx) {
            ...
            if (assetId.equals(SafeConstant.SAFE_FLAG)) {
                ...
                if (txCache.self) {
                    ...
                    valueView.setAmount(new AssetCoin(txCache.value.getValue(), (int) decimals));
                }
            }
            ...
            } else if (txCache.sent && confidenceType == ConfidenceType.PENDING && txCache.isIX) // Added for sending IX
            {
                ...
                messageView.setTextColor(colorInsignificant);
            } else if (!txCache.sent && txCache.value.compareTo(Transaction.MIN_NONDUST_OUTPUT) < 0 && !txCache.self) {
                extendMessageView.setVisibility(View.VISIBLE);
