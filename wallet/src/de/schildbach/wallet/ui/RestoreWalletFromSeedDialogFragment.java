...
public class RestoreWalletFromSeedDialogFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(final Bundle savedInstanceState) {
        ...
        builder.setPositiveButton(R.string.import_keys_dialog_button_import, new DialogInterface.OnClickListener() {
            ...
            public void onClick(final DialogInterface dialog, final int which) {
                ...
                List<String> words = new ArrayList<>(Arrays.asList(password.split("\\s")));
        ...
        passwordView.setFilters(new InputFilter[]{
                new InputFilter() {
                    public CharSequence filter(CharSequence src, int start, int end, Spanned dst, int dstart, int dend) {
                        ...
                        log.info("-------numOfWords = {}", numOfWords);
                        if (numOfWords >= NUMBER_OF_WORDS_IN_SEED && src.equals(" ")) {
                            return "";
                        }
                        if (src.toString().matches("[a-z\\s]+")) {
                            return src;
                        } else {
                          log.info("-------src = {}", src);
                        }
    ...
    private void restoreWalletFromSeed(final List<String> words) {
        try {
            ...
            ((MainActivity)activity).restoreWallet(WalletUtils.restoreWalletFromSeed(words, Constants.NETWORK_PARAMETERS));
            ...
            dialog.setPositiveButton(R.string.button_cancel, null);
            dialog.setNegativeButton(R.string.button_retry, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(final DialogInterface dialog, final int id) {
                    ((MainActivity)activity).handleRestoreWalletFromSeed();
                }
            });
            dialog.show();
    ...
    private int numOfWordsInPasswordView() {
        ...
        if (password.isEmpty()) {
            ...
            return password.split("\\s").length;
        }
    }
