...
public class BlockchainServiceImpl extends android.app.Service implements BlockchainService {
    ...
    private int notificationId = Constants.NOTIFICATION_START_ID_COINS_RECEIVED;
...
//            WalletBalanceWidgetProvider.updateWidgets(BlockchainServiceImpl.this, application.getWallet());
        ...
        public void onCoinsReceived(final Wallet wallet, final Transaction tx, final Coin prevBalance,
            ...
            ThreadPoolProxyFactory.getNormalThreadPoolProxy().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    int bestChainHeight = blockChain.getBestChainHeight();
                    Address address = WalletUtils.getWalletAddressOfReceived(tx, wallet);
                    ConfidenceType confidenceType = tx.getConfidence().getConfidenceType();
                    String assetId = tx.getAssetId();
                    Coin amount = tx.getValue(wallet, tx.getAssetId());
                    boolean isReceived = amount.signum() > 0;
                    boolean replaying = bestChainHeight < config.getBestChainHeightEver();
                    boolean isReplayedTx = confidenceType == ConfidenceType.BUILDING && replaying;
                    if (isReceived && !isReplayedTx) {
                        notifyCoinsReceived(address, amount, assetId, tx.isGetCandy());
                    }
                }
            });
        }
    ...
    private void notifyCoinsReceived(@Nullable final Address address, final Coin amount, String assetId, final boolean isGetCandy) {
        ...
        if (assetId.equalsIgnoreCase(SafeConstant.SAFE_FLAG)) {
            receivedAmount = config.getFormat().format(amount).toString();
        } else {
            BaseDaoImpl issueDao = new BaseDaoImpl(IssueData.class);
            try {
                IssueData issue = (IssueData) issueDao.queryForFirst("assetId", assetId);
                if (issue == null) return;
                receivedAmount = SafeUtils.getAssetAmount(amount, issue.decimals) + " " + issue.assetName;
            } catch (SQLException e) {
                receivedAmount = amount.toString();
            }
        }
        final String msg = getString(R.string.notification_coins_received_msg, receivedAmount) + msgSuffix;
        ...
        handler.post(new Runnable() {
            @Override
            public void run() {
                final String notifyAddress = label != null ? label : addressStr;
                final Notification.Builder notification = new Notification.Builder(BlockchainServiceImpl.this);
                notification.setSmallIcon(R.drawable.safe_notify);
                notification.setTicker(msg);
                notification.setContentTitle(msg);
                if (notifyAddress.length() > 0)
                    notification.setContentText(notifyAddress);

                Intent intent = new Intent(BlockchainServiceImpl.this, MainActivity.class);
                if (isGetCandy) { //如果是领取糖果发出通知
                    intent.setAction("to_candy_record");
                } else { //非糖果资产跳转到首页
                    intent.setAction("to_home_page");
                }
                notification.setContentIntent(PendingIntent.getActivity(BlockchainServiceImpl.this, 0, intent, 0));
                notification.setNumber(notificationId - Constants.NOTIFICATION_START_ID_COINS_RECEIVED);
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!connectivityNotificationEnabled || numPeers == 0) {
                        ...
                        Intent intent = new Intent(BlockchainServiceImpl.this, MainActivity.class);
                        intent.setAction("to_net_monitor");
                        notification.setContentIntent(PendingIntent.getActivity(BlockchainServiceImpl.this, 0, intent, 0));
                        notification.setWhen(System.currentTimeMillis());
    ...
    private final PeerDataEventListener blockchainDownloadListener = new PeerDataEventListener() {
        @Override
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock,
            ...
            int beforeHandBlockHeight = SafeConstant.getSafeBranchHeight() - SafeConstant.getBeforehandBlockBodies();
            if (blockChain.getChainHead().getHeight() >= beforeHandBlockHeight
                    && blockChain.getChainHead().getHeight() <= SafeConstant.getSafeBranchHeight()) {
                startDownloadBodies(blockChain.getChainHead().getHeader().getHash());
            }
     ...
     * 开始下载区块体
     */
    public void startDownloadBodies(Sha256Hash hash) {
        if (peerGroup != null && hash != null) {
            Peer downloadPeer = peerGroup.getDownloadPeer();
            downloadPeer.startDownloadBlockBodies(hash);
            for (Peer item : peerGroup.getConnectedPeers()) {
                item.startDownloadBlockBodies(hash);
            }
            for (Peer item : peerGroup.getPendingPeers()) {
                item.startDownloadBlockBodies(hash);
            }
        }
    }
    ...
    public void onCreate() {
        ...
        if (!blockChainFileExists) {
            ...
            EventBus.getDefault().postSticky(new EventMessage(EventMessage.TYPE_WALLET_RESET));
            config.setMustDownloadBlock(false);
        }

        try {
            blockStore = new SPVBlockStore(Constants.NETWORK_PARAMETERS, blockChainFile);
            blockStore.getChainHead(); // detect corruptions as early as possible
            // TODO: 2018/2/8 zhangmiao update 不跳过检查点
    ...
    public int onStartCommand(final Intent intent, final int flags, final int startId) {
        if (intent != null) {
            ...
            if (BlockchainService.ACTION_CANCEL_COINS_RECEIVED.equals(action)) {
                for (int id = Constants.NOTIFICATION_START_ID_COINS_RECEIVED; id < notificationId; id++) {
                    nm.cancel(id);
                }
                notificationId = Constants.NOTIFICATION_START_ID_COINS_RECEIVED;
                ...
                if (peerGroup != null) {
                    ...
                    log.info("peergroup not available, not broadcasting transaction {}" + tx);
                }
            }
        } else {
    ...
    // TODO: 2018/8/23 zhangmiao测试
    private void printBlockHeader() {
        StoredBlock chainHead = blockChain.getChainHead();
        ByteBuffer buffer = ByteBuffer.allocate(StoredBlock.COMPACT_SERIALIZED_SIZE);
        if (chainHead != null) {
            chainHead.serializeCompact(buffer);
            log.info("---height = {} ---buffer = {}", chainHead.getHeight(), CheckpointManager.BASE64.encode(buffer.array()));
        }
    }
