        ...
        if(processTxLockVote(pfrom, vote)){
            ...
            if(!mapLockRequestAccepted.containsKey(vote.txHash) && !mapLockRequestRejected.containsKey(vote.txHash)){
                if(!mapMasternodeOrphanVotes.containsKey(vote.vinMasternode.getOutpoint().getHash())){
                    mapMasternodeOrphanVotes.put(vote.vinMasternode.getOutpoint().getHash(), CommonUtils.currentTimeSeconds()+(60*10));
                }

                if(mapMasternodeOrphanVotes.get(vote.vinMasternode.getOutpoint().getHash()) > CommonUtils.currentTimeSeconds() &&
    ...
    */
    ...
    boolean processTxLockVote(Peer pnode, TransactionLockVote vote)
    {
        ...
        if (!mapTxLocks.containsKey(vote.txHash)){
            ...
            newLock.expiration = (int)(CommonUtils.currentTimeSeconds()+(60*60));
            newLock.timeout = (int)(CommonUtils.currentTimeSeconds()+(60*5));
