...
public class DarkSendSigner {
    ...
    {
        //int length = CommonUtils.BITCOIN_SIGNED_MESSAGE_HEADER.length()+strMessage.length();

        //byte dataToHash [] = (CommonUtils.BITCOIN_SIGNED_MESSAGE_HEADER_BYTES+strMessage).getBytes();
    ...
    public static boolean verifyMessage1(PublicKey pubkey, MasternodeSignature vchSig, byte[] message, StringBuilder errorMessage)
    {
        ...
        //ByteOutputStream bos = new ByteOutputStream(message.length + CommonUtils.BITCOIN_SIGNED_MESSAGE_HEADER_BYTES.length);
        //bos.write(CommonUtils.BITCOIN_SIGNED_MESSAGE_HEADER_BYTES);
        //bos.write(message);
