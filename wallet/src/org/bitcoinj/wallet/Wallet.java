...
import de.schildbach.wallet.ui.safe.utils.ThreadPoolProxyFactory;
...
public class Wallet extends BaseTaggableObject
    ...
    //zhangmiao update
    public Address noCacheAddress(KeyChain.KeyPurpose purpose) {
        keyChainGroupLock.lock();
        try {
            maybeUpgradeToHD();
            return keyChainGroup.noCacheFreshAddress(purpose);
        } finally {
            keyChainGroupLock.unlock();
        }
    }
    ...
    public List<ECKey> getAllEKeys() {
        keyChainGroupLock.lock();
        try {
            List<ECKey> chainKey = keyChainGroup.getActiveKeyChain().getAllEKeys();
            List<ECKey> importKey = keyChainGroup.getImportedKeys();
            chainKey.addAll(importKey);
            return chainKey;
        } finally {
            keyChainGroupLock.unlock();
        }
    ...
    public void receiveFromBlock(Transaction tx, StoredBlock block,
        ...
        try {
            if (isTransactionRelevant(tx)) {
                // TODO: 2018/8/22 已经达到Dash封存高度
                if (block.getHeight() >= SafeConstant.getDashDisabledHeight()) {
                    for (TransactionInput input : tx.getInputs()) {
                        TransactionOutput output = input.getConnectedOutput();
                        if (output != null && output.isDashTx()) {
                            return;
                        }
                    }
                }
            ...
            // TODO: 2018/8/22 已经达到Dash封存高度
            if (block.getHeight() >= SafeConstant.getDashDisabledHeight()) {
                if (tx.isBlockLastTx()) { //交易是否结束
                    int height = block.getHeight();
                    SafeUtils.handleCandy(height);  //处理糖果地址
                }
            }
        }
    }
    ...
    public Map<String, Coin> getHeightBalance(int height) {

        Map<String, Coin> map = new HashMap<>();
        List<TransactionOutput> mSpentOutput = new ArrayList<>();
        List<TransactionOutput> mUnspentOutput = new ArrayList<>();
        Map<Sha256Hash, Transaction> allTx = new HashMap<>();
        allTx.putAll(transactions);
        Map<Sha256Hash, Transaction> filterTx = new HashMap<>(); //过滤交易
        for (Map.Entry<Sha256Hash, Transaction> entry : allTx.entrySet()) {
            Sha256Hash hash = entry.getKey();
            Transaction itemTx = entry.getValue();
            TransactionConfidence confidence = itemTx.getConfidence();
            ...
            if (confidenceType == TransactionConfidence.ConfidenceType.BUILDING) {
                if (confidence.getAppearedAtChainHeight() <= height) {
                    filterTx.put(hash, itemTx);
                }
            }
        }

        for (Map.Entry<Sha256Hash, Transaction> entry : filterTx.entrySet()) {
            Transaction itemTx = entry.getValue();
            for (TransactionInput input : itemTx.getInputs()) {
                TransactionOutPoint outPoint = input.getOutpoint();
                Transaction tx = filterTx.get(outPoint.getHash());
                if (tx != null) {
                    TransactionOutput output = tx.getOutput(outPoint.getIndex());
                    if (output != null) {
                        mSpentOutput.add(output);
                    }
                }
            }
        }
        ...
        for (Map.Entry<Sha256Hash, Transaction> entry : filterTx.entrySet()) {
            Transaction itemTx = entry.getValue();
            for (TransactionOutput output : itemTx.getOutputs()) {
                //达世币封存，这里忽略
                if (!mSpentOutput.contains(output) && output.isMineOrWatched(this) && !output.isDashTx() && output.isSafeTx()) {
                    mUnspentOutput.add(output);
                }
            }
        }

        if (mUnspentOutput.size() > 0) {
            List<ECKey> allEKeys = getAllEKeys();
            for (ECKey key : allEKeys) {
                Address address = key.toAddress(Constants.NETWORK_PARAMETERS);
                Coin value = getAddressBalance(mUnspentOutput, address);
                if (value.getValue() >= 100000000L) {
                    map.put(address.toBase58(), value);
                }
            }
        }
        return map;
    ...
    public Coin getAddressBalance(Collection<TransactionOutput> mUnspentOutput, Address address) {
        List<TransactionOutput> candidates = new ArrayList<>(mUnspentOutput.size());
        for (TransactionOutput output : mUnspentOutput) {
            Transaction transaction = checkNotNull(output.getParentTransaction());
            if (!transaction.isMature())
                continue;
            candidates.add(output);
        }
        List<TransactionOutput> filterCandidates = new ArrayList<>();
        for (TransactionOutput output : candidates) {
            Address outAddress = output.getAddressFromP2PKHScript(Constants.NETWORK_PARAMETERS);
            if (outAddress != null) {
                if (address.toBase58().equals(outAddress.toBase58())) {
                    filterCandidates.add(output);
                }
            }
        }
        Coin value = Coin.ZERO;
        for (TransactionOutput output : filterCandidates) {
...
//        log.info("---------valueSentFromMe = {}, valueSentToMe = {}", valueSentFromMe, valueSentToMe);
...
//                        log.info("---------------myUnspents 1--------");
            ...
            if (valueSentFromMe.isZero() && valueSentFromMe.isZero() && tx.isGetCandy()) {
                //暂时处理其他钱包领取糖果，导致钱包重置的问题。
            } else {
    ...
    private void updateForSpends(Transaction tx, boolean fromChain) throws VerificationException {
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                ...
                if (!transactions.containsKey(input.getOutpoint().getHash())) {
                    input.disconnect();
                }
...
//            log.info("------- commitTx of {}", tx.getHashAsString());
    ...
    public void receivedThread(final Transaction tx) {
        ThreadPoolProxyFactory.getReceivedThreadPoolProxy().execute(new Runnable() {
            @Override
            public void run() {
                try {
                    SafeUtils.saveMeWalletAssetTx(tx);
                } catch (Exception e) {
                }
            }
        });
...
//                            log.info("---------------myUnspents 4--------");
    ...
    public int getWalletSeenHeight() {
        return new Integer(lastBlockSeenHeight);
    }
    ...
    public Coin getBalance(BalanceType balanceType) {
        ...
        try {
            if (balanceType == BalanceType.AVAILABLE || balanceType == BalanceType.AVAILABLE_SPENDABLE) {
                ...
                Coin value = Coin.ZERO;
                for (TransactionOutput output : all) value = value.add(output.getValue());
                return value;
            } else {
     ...
     * @return
     */
    public boolean isOutputLocked(TransactionOutput output) {
        int nCurrentHeight = getWalletSeenHeight();
        int startSposHeight = params.getStartSposHeight();
        int txHeight = getTxHeight(output);
        if (output.getLockHeight() > 0) {
            if (txHeight < startSposHeight) {
                if (output.getLockHeight() > startSposHeight) { //采用POW和SPOS
                    int sposLaveHeight = ((int)output.getLockHeight()  - startSposHeight) * (CoinDefinition.TARGET_SPACING / CoinDefinition.SPOS_TARGET_SPACING);
                    int neededHeight = startSposHeight + sposLaveHeight;
                    return nCurrentHeight < neededHeight;
                } else { //采用POW
                    return nCurrentHeight < output.getLockHeight();
                }
            } else { //采用SPOS
                return nCurrentHeight < output.getLockHeight();
            }
    ...
    public int getTxHeight(TransactionOutput output) {
     ...
     * Returns the balance of this wallet as calculated by the provided balanceType.
     */
    public Coin getBalance(BalanceType balanceType, String assetId, Address address) {
        lock.lock();
        try {
            if (balanceType == BalanceType.AVAILABLE || balanceType == BalanceType.AVAILABLE_SPENDABLE) {
                List<TransactionOutput> allOutput = calculateAllSpendCandidates(true, balanceType == BalanceType.AVAILABLE_SPENDABLE, assetId, address);
                List<TransactionOutput> candidates = new ArrayList<>();
                for (TransactionOutput output : allOutput) { //可用时需要过滤锁定的金额
                    if (isOutputLocked(output)) {
                        continue;
                    } else {
                        candidates.add(output);
                    }
                }
                if (assetId != null && assetId.equals(SafeConstant.SAFE_FLAG)) {
                    CoinSelection selection = coinSelector.select(NetworkParameters.MAX_MONEY, candidates);
                    return selection.valueGathered;
                } else {
                    CoinSelection selection = coinSelector.select(NetworkParameters.MAX_ASSET_MONEY, candidates);
                    return selection.valueGathered;
                }
            } else if (balanceType == BalanceType.ESTIMATED || balanceType == BalanceType.ESTIMATED_SPENDABLE) {
                List<TransactionOutput> allOutput = calculateAllSpendCandidates(false, balanceType == BalanceType.ESTIMATED_SPENDABLE, assetId, address);
                Coin value = Coin.ZERO;
     ...
     * Returns the balance of this wallet as calculated by the provided balanceType.
     */
    public Coin getLockBalance(BalanceType balanceType, String assetId, Address address) {
        lock.lock();
        try {
            if (balanceType == BalanceType.AVAILABLE || balanceType == BalanceType.AVAILABLE_SPENDABLE) {
                List<TransactionOutput> allOutput = calculateAllSpendCandidates(true, balanceType == BalanceType.AVAILABLE_SPENDABLE, assetId, address);
                List<TransactionOutput> candidates = new ArrayList<>();
                for (TransactionOutput output : allOutput) {  //只计算锁定的金额
                    if (isOutputLocked(output)) {
                        candidates.add(output);
                    }
                }
     ...
     * followed by {@link Wallet} and returning the requests transaction object.
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            // TODO: 2018/5/15 zhangmiao update
            Coin addedFee = Coin.ZERO;

            for (TransactionOutput output : req.tx.getOutputs()) {
                if (output.isSafeTx()) {
                    value = value.add(output.getValue());
                } else {
                    byte[] mBytes = output.getReserve();
                    ...
                    addedFee = addedFee.add(Coin.valueOf(minFee));
                }
            }
            value = value.add(addedFee);
            ...
            if (req.ensureMinRequiredFee && !req.emptyWallet) { // Min fee checking is handled later for emptyWallet.
                int opReturnCount = 0;
                for (TransactionOutput output : req.tx.getOutputs()) {
                    boolean isDust = output.isDust();
                    if (output.isSafeTx() && isDust)
            ...
            List<TransactionOutput> unpentOutput = calculateAllSpendCandidates(true, req.missingSigsMode == MissingSigsMode.THROW, SafeConstant.SAFE_FLAG, req.address);

            // TODO: 2018/3/20 zhangmiao update 过滤锁定交易
            List<TransactionOutput> candidates = new ArrayList<>();
            for (TransactionOutput output : unpentOutput) {
                if (isOutputLocked(output)) {
                    continue;
                } else if (getWalletSeenHeight() == SafeConstant.getDashDisabledHeight() - 1) { //过滤达世交易
                    if (output.isDashTx()) {
                        continue;
                    } else {
                        candidates.add(output);
            ...
            Coin instantMaxValue = Coin.valueOf(100000000000L);

            if (!req.emptyWallet) {
                FeeCalculation feeCalculation = null;
                if (req.useInstantSend) {
                    Coin tempCoin = null; //最后一个超过1000SAFE的输入对应金额
                    List<TransactionOutput> tempCandidates = new ArrayList<>();
                    tempCandidates.addAll(candidates);
                    for (TransactionOutput output : candidates) {
                        if (!output.getValue().isLessThan(instantMaxValue)) {
                            tempCoin = output.getValue();
                            tempCandidates.remove(output);
                        }
                    }
                    try {
                        // This can throw InsufficientMoneyException.
                        feeCalculation = calculateFee(req, value, originalInputs, req.ensureMinRequiredFee, tempCandidates);
                    } catch (InsufficientMoneyException insufficientMoneyException) {
                        if (tempCoin != null) {//这里是余额足，但是没有满足交易的输出
                            throw new InsufficientMoneyException(tempCoin, false, true);
                        } else {
                            throw insufficientMoneyException;
                        }
                    }
                } else {
                    feeCalculation = calculateFee(req, value, originalInputs, req.ensureMinRequiredFee, candidates);
                }
                ...
                log.info("clear emptying {}", bestCoinSelection.valueGathered.toFriendlyString());
            }

            // TODO: 2018/2/7 zhangmiao update
            if (req.useInstantSend) { //输出金额不能超过1000SAFE
                if (!value.isLessThan(instantMaxValue)) {
                    throw new InsufficientMoneyException(value, true, false);
                }
            }
            ...
            for (TransactionOutput output : bestCoinSelection.gathered) {
                if (req.useInstantSend) {
                    totalInputValue = totalInputValue.add(output.getValue());
                }
                req.tx.addInput(output);
            }

            if (!calcTxDepth(req.tx)) {
                throw new InsufficientMoneyException(Coin.ZERO, false, false, true);
            }

            if (req.useInstantSend) { //总输入金额不能超时1000SAFE
                if (!totalInputValue.isLessThan(instantMaxValue)) {
                    throw new InsufficientMoneyException(totalInputValue, false, true);
                }
            }
            ...
            // TODO: 2018/6/15 zhangmiao add
            if (req.appCommand == SafeConstant.CMD_ISSUE) { //处理第一个输入地址为资产的管理地址
                TransactionInput input = req.tx.getInputs().get(0);//获取到第一个输入
                TransactionOutPoint outPoint = input.getOutpoint();//获取第一个输入对应的上一笔交易输出point
                Sha256Hash txId = outPoint.getHash();//获取到上一笔交易id
                long index = outPoint.getIndex();//outpoint位置
                Transaction tx = getTransaction(txId);//获取上一笔交易
                TransactionOutput output = tx.getOutput(index);//这里获取到第一个输入对应的上一笔交易的输出
                Address assetAddress = output.getAddressFromP2PKHScript(params);//获取到上一笔交易输出对应地址
                List<TransactionOutput> newOutputs = new ArrayList();
                for (TransactionOutput item : req.tx.getOutputs()) {//遍历当前交易的输出（这里输出最多有三个，发布资产的输出，找零输出，发布糖果输出）
                    SafeReserve reserve = item.getSafeReserve();//获取到预留字段
                    if (reserve.isIssue()) {//如果是发布资产
                        Address outAddress = item.getAddressFromP2PKHScript(params);//获取到发布资产输出地址，如果它等于第一个输入地址则直接添加，否则将地址替换为第一个输入地址
                        if (!assetAddress.toBase58().equals(outAddress.toBase58())) {
                            Script script = ScriptBuilder.createOutputScript(assetAddress);
                            newOutputs.add(new TransactionOutput(params, req.tx, item.getValue(), script.getProgram(), item.getLockHeight(), item.getReserve()));
                        } else {
                            newOutputs.add(item);
                        }
                    } else {
                ...
                req.tx.clearOutputs();//清空输出
                for (TransactionOutput item : newOutputs)//重新设置输出
                    req.tx.addOutput(item);
            } else if (req.appCommand == SafeConstant.CMD_GET_CANDY) { //处理最后一个输入地址为糖果的黑洞地址
                List<TransactionInput> sortInputs = new ArrayList();
                sortInputs.addAll(req.tx.getInputs());
                req.tx.clearInputs();
                int removeIndex = 0;
                for (int i = 0; i < sortInputs.size(); i++) {
                    TransactionInput txIn = sortInputs.get(i);
                    if (txIn.getConnectedOutput() == null) {
                        removeIndex = i;
                        continue;
                    }
                    //过滤发放糖果地址
                    TransactionOutput output = txIn.getConnectedOutput();
                    if (Address.fromBase58(Constants.NETWORK_PARAMETERS, SafeConstant.CANDY_BLACK_HOLE_ADDRESS).equals(output.getAddressFromP2PKHScript(Constants.NETWORK_PARAMETERS))) {
                        removeIndex = i;
                        continue;
                    }
                }
                TransactionInput lastIn = sortInputs.remove(removeIndex);
                sortInputs.add(lastIn);
                for (TransactionInput input : sortInputs)
                    req.tx.addInput(input);
            }

            if (bestChangeOutput != null) {
                req.tx.addOutput(bestChangeOutput);
                log.info("  with {} change  address {} ", bestChangeOutput.getValue(), bestChangeOutput.getAddressFromP2PKHScript(Constants.NETWORK_PARAMETERS));
            }
            ...
            req.completed = true;
            log.info("  completed: {} {} ", req.tx.unsafeBitcoinSerialize().length, req.tx);
        ...
        try {
            ...
            if (vUTXOProvider == null) {
                ...
                for (TransactionOutput output : myUnspents) {
                    // TODO: 2018/8/22 已经达到Dash封存高度
                    if (lastBlockSeenHeight >= SafeConstant.getDashDisabledHeight()) {
                        if (output.isDashTx()) {
                            continue;
                        }
                    }
    ...
    public List<TransactionOutput> calculateAllSpendCandidatesAndAsset(boolean excludeImmatureCoinbases, boolean excludeUnsignable) {
        lock.lock();
        try {
            List<TransactionOutput> candidates;
            if (vUTXOProvider == null) {
    ...
    public List<TransactionOutput> calculateAllSpendCandidates(boolean excludeImmatureCoinbases, boolean excludeUnsignable, String assetId, Address address) {
        lock.lock();
        try {
            List<TransactionOutput> candidates;
            if (vUTXOProvider == null) {
            ...
            List<TransactionOutput> filterCandidates = new ArrayList<>();
            for (TransactionOutput output : candidates) {
                if (!TextUtils.isEmpty(assetId) && address == null) { //只查找assetId
                    if (assetId.equals(SafeConstant.SAFE_FLAG) && output.isSafeTx()) { //safe资产交易
                        filterCandidates.add(output);
                    } else { //其他资产交易
                        if (output.isCurrentAssetTx(assetId)) {
                            filterCandidates.add(output);
                        }
                    }
                } else if (TextUtils.isEmpty(assetId) && address != null) {  //只查找address
                    Address txAddress = output.getAddressFromP2PKHScript(Constants.NETWORK_PARAMETERS);
                    if (txAddress!=null && address.toBase58().equals(txAddress.toBase58())) {
                        filterCandidates.add(output);
                    }
                } else if (!TextUtils.isEmpty(assetId) && address != null) {  //assetId和address都查找
                    Address txAddress = output.getAddressFromP2PKHScript(Constants.NETWORK_PARAMETERS);
                    if (txAddress != null && address.toBase58().equals(txAddress.toBase58())) {
                        SafeReserve reserve = SafeUtils.parseReserve(output.getReserve());
                        if (assetId.equals(SafeConstant.SAFE_FLAG) && reserve.isSafeTx()) { //safe资产交易
                            filterCandidates.add(output);
                        } else { //其他资产交易
                            if (output.isCurrentAssetTx(assetId)) {
                                filterCandidates.add(output);
...
//            log.info("------ value = {}, fees = {}", value, fees);

            valueNeeded = value.add(fees);
//            log.info("------valueNeeded = {}", valueNeeded);
            ...
            if (change.signum() > 0) {
                ...
                if (req.ensureMinRequiredFee && changeOutput.isDust()) {
                    ...
                    int outSize = VarInt.sizeOf(req.tx.getOutputs().size());
                    int inSize = VarInt.sizeOf(req.tx.getOutputs().size() - 1);
                    size += changeOutput.unsafeBitcoinSerialize().length + outSize - inSize;
//                    log.info("------change outSize = {}, inSize ={}, size = {}", outSize, inSize, size);
...
//            log.info("------unsign size = {}", size);
            size += estimateBytesForSigning(selection);
//            log.info("------safe sign size = {}", size);
            if (req.assetSignSize > 0) {
                size += req.assetSignSize;
//                log.info("------asset sign size = {}", size);
            }
    ...
    public int estimateBytesForSigning(CoinSelection selection) {
        int size = 0;
        for (TransactionOutput output : selection.gathered) {
            try {
    ...
    //
    private int estimateBytesForSigning(SendRequest req, List<TransactionInput> assetInputs) {
        int size = 0;
        for (TransactionInput in : assetInputs) {
            Script scriptPubKey = in.getConnectedOutput().getScriptPubKey();
            KeyBag maybeDecryptingKeyBag = new DecryptingKeyBag(this, req.aesKey);
            RedeemData redeemData = in.getConnectedRedeemData(maybeDecryptingKeyBag);
            Script redeemScript = redeemData.redeemScript;
            size += scriptPubKey.getNumberOfBytesRequiredToSpend(null, redeemScript);
        }
        return size;
    }
    ...
    public boolean calcTxDepth(Transaction tx) {
        maxDepth = FINAL_DEPTH;
        calcPrevDepth(tx, FINAL_DEPTH);
        if (maxDepth > 0) { //可以创建
            return true;
        } else { //不可以创建
            return false;
    ...
    public final int FINAL_DEPTH = 25;

    public int maxDepth = 0;

    public void calcPrevDepth(Transaction tx, int depth) {
        boolean isExistPrev = false;
        for (TransactionInput input : tx.getInputs()) {
            TransactionOutPoint outPoint = input.getOutpoint();
            Transaction prevTx = pending.get(outPoint.getHash());
            if (prevTx != null) {
                calcPrevDepth(prevTx, depth - 1);
                isExistPrev = true;
            }
        }
        if (!isExistPrev) {
...
//    public void importTestKey(){
//        DumpedPrivateKey dpKey = DumpedPrivateKey.fromBase58(Constants.NETWORK_PARAMETERS, "XDzwPKFis3eaePgS48CruCUKv1jEMhMMgpBBQS69NpMRF1Ra26Vd");
//        importKey(dpKey.getKey());
//    }

    public void killRejectTx(final String txId) {
        lock.lock();
        try {
            Transaction mTx = getTransactionPool(Pool.PENDING).get(Sha256Hash.wrap(txId));
            killTxns(ImmutableSet.of(mTx), null);
        } finally {
            lock.unlock();
        }
    }
