...
public class PeerGroup implements TransactionBroadcaster {
    ...
    private class PeerStartupListener implements PeerConnectedEventListener, PeerDisconnectedEventListener {
        ...
        public void onPeerConnected(Peer peer, int peerCount) {
            if (peer.getPeerVersionMessage().subVer.equals("/Safe Core:1.0.0/") || peer.getPeerVersionMessage().subVer.equals("/Safe Core:1.0.1/")) {
                handlePeerDeath(peer, null);
            } else {
                if(SafeConstant.getLastBlockHeight() >= Constants.NETWORK_PARAMETERS.getForbidOldVersionHeight()) {
                    if (peer.getPeerVersionMessage().subVer.equals("/Safe Core:2.5.0/") || peer.getPeerVersionMessage().subVer.equals("/Safe Core:2.5.1/")) {
                        handlePeerDeath(peer, null);
                    }
                } else {
                    handleNewPeer(peer);
    ...
    private Runnable triggerConnectionsJob = new Runnable() {
        ...
        private final static long MIN_PEER_DISCOVERY_INTERVAL = 100L;
