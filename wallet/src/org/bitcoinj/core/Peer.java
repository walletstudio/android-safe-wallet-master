...
public class Peer extends PeerSocketHandler {
    ...
    protected void processMessage(Message m) throws Exception {
        ...
        {
            log.warn("{}: Received unhandled message: {}", this, m.toString());
        }
    }
    ...
    protected void processTransaction(final Transaction tx) throws VerificationException {
        ...
        try {
            ...
            if (tx.getConfidence().getConfidenceType() == TransactionConfidence.ConfidenceType.BUILDING) {
                int height = tx.getConfidence().getAppearedAtChainHeight();
                if (height >= SafeConstant.getDashDisabledHeight()) {
...
//            if(!instantSend.mapTxLockVotes.containsKey(item.hash))
                        ...
                        if (!pendingBlockDownloads.contains(item.hash)) {
                            if (vPeerVersionMessage.isBloomFilteringSupported() && useFilteredBlocks) {
                                if (Constants.TEST) {
                                    ...
                                    if (SafeConstant.getLastBlockHeight() >= SafeConstant.getSafeBranchHeight()) {
                                        getdata.addBlock(item.hash);
                                    } else {
        ...
        try {
            if (secondsSinceEpoch == 0) {
                ...
                if (Constants.TEST) {
                    ...
                    if (blockChain != null && blockChain.getChainHead().getHeight() >= (SafeConstant.getSafeBranchHeight() - SafeConstant.getBeforehandBlockBodies())) {
                        downloadBlockBodies = true;
                    } else {
    ...
    //开始下载区块体
    public void startDownloadBlockBodies(Sha256Hash toHash) {
        if (!downloadBlockBodies) {
            downloadBlockBodies = true;
            lock.lock();
            try {
