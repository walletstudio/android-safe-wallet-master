...
public class Block extends Message {
    ...
    public Coin getBlockInflation(int height, long nPrevBits, boolean fSuperblockPartOnly) {
        //return CommonUtils.toNanoCoins(50, 0).shiftRight(height / context.getSubsidyDecreaseBlockCount());
        //return /*CommonUtils.toNanoCoins(*/CoinDefinition.GetBlockReward(height)/*, 0)*/;
        double dDiff;
        long nSubsidyBase;
     ...
     * @param height
     * @param nPrevBits
     * @param fSuperblockPartOnly
     * @return
     */
    public Coin getBlockInflationCoin(int height, long nPrevBits, boolean fSuperblockPartOnly) {
        //return CommonUtils.toNanoCoins(50, 0).shiftRight(height / context.getSubsidyDecreaseBlockCount());
        //return /*CommonUtils.toNanoCoins(*/CoinDefinition.GetBlockReward(height)/*, 0)*/;
        ...
        double nSubsidyBase;
        int nPrevHeight = height - 1;
//        log.info("-------nPrevHeight = {}, nPrevBits = {}", nPrevHeight, nPrevBits);
        ...
        if (nPrevHeight < 5465) {
            ...
            if (nPrevHeight > params.getCriticalHeight()) {
                if (nSubsidyBase > 5)
                    nSubsidyBase = 5;
        ...
        long longSubsidyBase = (long) (nSubsidyBase * SafeUtils.getRealDecimals(8));

        // yearly decline of production by ~7.1% per year, projected ~18M coins max by year 2050+.
        for (int i = params.getSubsidyDecreaseBlockCount(); i <= nPrevHeight; i += params.getSubsidyDecreaseBlockCount()) {
            longSubsidyBase = longSubsidyBase - (longSubsidyBase / 14);
        }

        // Hard fork to reduce the block reward by 10 extra percent (allowing budget/superblocks)
        long newLongSubsidyBase = (nPrevHeight > params.getBudgetPaymentsStartBlock()) ? longSubsidyBase/10 : 0;

//        log.info("-------coin = {}", longSubsidyBase - newLongSubsidyBase);

        return fSuperblockPartOnly ? Coin.valueOf(longSubsidyBase) : Coin.valueOf(longSubsidyBase - newLongSubsidyBase);
    }
...
//
//        long nSubsidy = CoinDefinition.SPOS_MAX_INCENTIVES;
//
//        int nPrevHeight = height - 1;
//
//        int nSubsidyHalvingInterval = params.getSubsidyDecreaseBlockCount() * (CoinDefinition.TARGET_SPACING / CoinDefinition.SPOS_TARGET_SPACING);
//
//        // yearly decline of production by ~7.1% per year, projected ~18M coins max by year 2050+.
...
//        }
//
//        nSubsidy = nSubsidy / (CoinDefinition.TARGET_SPACING / CoinDefinition.SPOS_TARGET_SPACING);
//
//        // Hard fork to reduce the block reward by 10 extra percent (allowing budget/superblocks)
//        long nSuperblockPart = (nPrevHeight > params.getBudgetPaymentsStartBlock()) ? nSubsidy / 10 : 0;
//        return fSuperblockPartOnly ? Coin.valueOf(nSuperblockPart) : Coin.valueOf(nSubsidy - nSuperblockPart);
//    }
    ...
    public Coin getSposBlockInflationCoin(int height, boolean fSuperblockPartOnly)
    {
        ...
        int nStartDecrementHeight = g_nStartSPOSHeight + nOffset * (CoinDefinition.TARGET_SPACING / CoinDefinition.SPOS_TARGET_SPACING);
        int nSubsidyHalvingInterval = params.getSubsidyDecreaseBlockCount() * (CoinDefinition.TARGET_SPACING / CoinDefinition.SPOS_TARGET_SPACING);
        // yearly decline of production by ~7.1% per year, projected ~18M coins max by year 2050+.
    ...
    public BigInteger getWork() throws VerificationException {
        ...
        if (target.signum() == 0) { //target等于0 代表SPOS直接返回
            return target;
        } else {
    ...
    public void verifyHeader(int height) throws VerificationException {
        ...
        if (height > 0 && height >= params.startSposHeight) {
            checkSpos();
        } else {
    ...
    //检查SPOS难度
    public void checkSpos() {
        BigInteger target = Utils.decodeCompactBits(difficultyTarget);
        if (target.signum() != 0) {
            throw new VerificationException("SPOS_Error CheckSPOSBlock(): difficultyTarget not equal to 0, target : %s" + target.toString());
        }
        checkCoinBase();
    }

    //检查CoinBase交易
    public void checkCoinBase() {
        List<Transaction> transactions = getTransactions();
        if (transactions != null && transactions.size() > 0) {
            Transaction tx = transactions.get(0);
            if (tx.isCoinBase()) {
                TransactionOutput output = tx.getOutput(0);
                byte[] mReserve = output.getReserve();
                if (!SafeUtils.validSposReserve(output, mReserve)) {
                    throw new VerificationException("SPOS_CoinBase_Reserve_Error validSposReserve");
                }
            }
        }
    }
