...
public class Transaction extends ChildMessage {
    ...
    private boolean blockLastTx = false;

    public boolean isBlockLastTx() {
        return blockLastTx;
    }

    public void setBlockLastTx(boolean blockLastTx) {
        this.blockLastTx = blockLastTx;
    }
     ...
     * @param parent  Whether to retain the backing byte array for quick reserialization.
     ...
     * Calculates the sum of the outputs that are sending coins to a key in the wallet.
     */
    public Coin getValueSentToMe(TransactionBag transactionBag, String assetId) {
        // This is tested in WalletTest.
        Coin v = Coin.ZERO;
        for (TransactionOutput o : outputs) {
            if (!o.isMineOrWatched(transactionBag)) continue;
            if (assetId.equals(SafeConstant.SAFE_FLAG) && o.isSafeTx()) { //safe资产交易
                v = v.add(o.getValue());
            } else if (o.isCurrentAssetTx(assetId)) { //其他资产交易
                v = v.add(o.getValue());
            }
        }
        return v;
     ...
     * @return sum of the inputs that are spending coins with keys in the wallet
     */
    public Coin getValueSentFromMe(TransactionBag wallet, String assetId) throws ScriptException {
        // This is tested in WalletTest.
        Coin v = Coin.ZERO;
        for (TransactionInput input : inputs) {
            // This input is taking value from a transaction in our wallet. To discover the value,
...
//            log.info("--------connected = {}", connected);
            if (assetId.equals(SafeConstant.SAFE_FLAG) && connected.isSafeTx()) { //safe资产交易
                v = v.add(connected.getValue());
            } else if (connected.isCurrentAssetTx(assetId)) { //其他资产交易
                v = v.add(connected.getValue());
            }
        }
        return v;
     ...
     * Returns the difference of {@link Transaction#getValueSentToMe(TransactionBag)} and {@link Transaction#getValueSentFromMe(TransactionBag)}.
     */
    public Coin getValue(TransactionBag wallet, String assetId) throws ScriptException {
        return getValueSentToMe(wallet, assetId).subtract(getValueSentFromMe(wallet, assetId));
    }
    ...
    public Coin getFee() {
        ...
        for (TransactionInput input : inputs) {
            ...
            if (output != null && output.isSafeTx()) {
    ...
    protected void parse() throws ProtocolException {
        ...
        for (long i = 0; i < numOutputs; i++) {
            TransactionOutput output = new TransactionOutput(params, this, payload, cursor, serializer, version);
            outputs.add(output);
            // TODO: 2018/1/24 zhangmiao update
            long scriptLen = readVarInt(8);
            if (version >= SafeConstant.BRANCH_TX_VERSION) {
                int size = (int) scriptLen + 8;
                long reserveLen = readVarInt(size);
                optimalEncodingMessageSize += 8 + VarInt.sizeOf(scriptLen) + scriptLen + 8 + VarInt.sizeOf(reserveLen) + reserveLen;
                cursor += reserveLen;
            } else {
...
//                        s.append(" hash160:");
//                        s.append(CommonUtils.HEX.encode(scriptPubKey.getPubKeyHash()));
//                    }
//                }
//                if (in.hasSequence()) {
...
//            }
        ...
        if (fee != null) {
            ...
            s.append("     fee  ").append(fee.multiply(1000).divide(size).toFriendlyString()).append("/kB, ")
                    .append(fee.value).append(" for ").append(size).append(" bytes\n");
        }
        if (purpose != null)
     ...
     * complete and after every input is added with {@link TransactionOutput} and every output is added with
     * {@link TransactionOutput}, a {@link TransactionSigner} must be used to finalize the transaction and finish the inputs
     ...
     * you won't normally need to use it unless you're doing unusual things.
     */
    public TransactionOutput addOutput(Coin value, Script script, long nUnlockHeight, byte[] vReserve) {
        return addOutput(new TransactionOutput(params, this, value, script.getProgram(), nUnlockHeight, vReserve));
    }
    ...
    public boolean getLocked(TransactionBag transactionBag) {
        String assetId = getAssetId();
        Coin value = getValue(transactionBag, assetId);
        boolean sent = value.signum() < 0;
        if (sent) {
            for (TransactionOutput item : getOutputs()) {
                if (item.getLockHeight() > 0) {
    ...
    public boolean isSafeTx() {
        for (TransactionOutput output : getOutputs()) {
            SafeReserve safeReserve = SafeUtils.parseReserve(output.getReserve());
    ...
    public TransactionInput getLastInput(){
        return getInput(getInputs().size() - 1);
    }

    public String getAssetId() {
        String assetId = SafeConstant.SAFE_FLAG;
        for (TransactionOutput output : getOutputs()) {
            SafeReserve reserve = SafeUtils.parseReserve(output.getReserve());
            if (reserve.isIssue()) {
                assetId = SafeUtils.generateAssetId(reserve.getIssueProtos());
                break;
            } else if (reserve.isAddIssue() || reserve.isTransfer() || reserve.isAssetChange()) {
                SafeProtos.CommonData protos = reserve.getCommonProtos();
                assetId = SafeUtils.hash256ToAssetId(protos.getAssetId().toByteArray());
                break;
     ...
     * @return
     */
    public Coin getAssetTransferValue() {
        for (TransactionOutput output : getOutputs()) {
            SafeReserve reserve = SafeUtils.parseReserve(output.getReserve());
            if (reserve.isTransfer()) {
                return Coin.valueOf(reserve.getCommonProtos().getAmount());
            }
        }
        return Coin.ZERO;
    }
     ...
     * @return
     */
    public boolean isDashUnspent(boolean sent) {
