...
public class TransactionOutput extends ChildMessage {
    ...
    // TODO: 2018/1/23 zhangmiao update
    private long nUnlockHeight;
    private int reserveLen;
    private byte[] vReserve;
    private SafeReserve mSafeReserve;
    ...
    // TODO: 2018/2/26 zhangmiao update
    public TransactionOutput(NetworkParameters params, @Nullable Transaction parent, byte[] payload, int offset, MessageSerializer serializer, long version) throws ProtocolException {
        super(params, payload, offset, parent, serializer, UNKNOWN_LENGTH, version);
        availableForSpending = true;
    }
    ...
    public TransactionOutput(NetworkParameters params, @Nullable Transaction parent, Coin value, byte[] scriptBytes) {
        ...
        this.reserveLen = SafeConstant.SAFE_FLAG.length();
        this.vReserve = SafeConstant.SAFE_FLAG.getBytes();
        length = 8 + VarInt.sizeOf(this.reserveLen) + this.reserveLen;
    }

    public TransactionOutput(NetworkParameters params, @Nullable Transaction parent, Coin value, byte[] scriptBytes, long nUnlockHeight) {
        this(params, parent, value, scriptBytes, nUnlockHeight, SafeConstant.SAFE_FLAG.getBytes());
    }

    public TransactionOutput(NetworkParameters params, @Nullable Transaction parent, Coin value, byte[] scriptBytes, long nUnlockHeight, byte[] vReserve) {
        super(params);
        // Negative values obviously make no sense, except for -1 which is used as a sentinel value when calculating
        // SIGHASH_SINGLE signatures, so unfortunately we have to allow that here.
        checkArgument(value.signum() >= 0 || value.equals(Coin.NEGATIVE_SATOSHI), "Negative values not allowed");
        SafeReserve safeReserve = SafeUtils.parseReserve(vReserve);
        if (safeReserve.isSafeTx()) {
            checkArgument(!params.hasMaxMoney() || value.compareTo(params.getMaxMoney()) <= 0, "Values larger than MAX_MONEY not allowed");
        }
        this.value = value.value;
        this.scriptBytes = scriptBytes;
        ...
        availableForSpending = true;
        length = 8 + VarInt.sizeOf(scriptBytes.length) + scriptBytes.length;
        this.nUnlockHeight = nUnlockHeight;
        this.reserveLen = vReserve.length;
        this.vReserve = vReserve;
        length = 8 + VarInt.sizeOf(this.reserveLen) + this.reserveLen;
    }
    ...
    protected void parse() throws ProtocolException {
        ...
        // TODO: 2018/1/23 zhangmiao update，解析交易parent没赋值，所以直接在构造方法里面传递
        if (txVersion >= SafeConstant.BRANCH_TX_VERSION) {
            nUnlockHeight = readInt64();
            reserveLen = (int) readVarInt();
            length = cursor - offset + reserveLen;
            vReserve = readBytes(reserveLen);
        }
    }
    ...
    protected void bitcoinSerializeToStream(OutputStream stream) throws IOException {
        ...
        // TODO: 2018/1/23 zhangmiao update，序列化交易时，直接在WalletProtobufSerializer里面赋值
//        log.info("----txVersion = {}", txVersion);
        if (getParentTransaction() != null  && getParentTransaction().getVersion() >= SafeConstant.BRANCH_TX_VERSION) {
            if (vReserve != null && vReserve.length > 0) {
                Utils.int64ToByteStreamLE(nUnlockHeight, stream);
                stream.write(new VarInt(vReserve.length).encode());
                stream.write(vReserve);
            }
        }
    }
    ...
    public boolean isMineOrWatched(TransactionBag transactionBag) {
        boolean isMine = isMine(transactionBag);
        boolean isWatched = isWatched(transactionBag);
        return isMine || isWatched;
    }
    ...
    public String toString() {
        try {
            ...
            if (vReserve != null) {
                buf.append(" lockHeight:").append(nUnlockHeight);
                buf.append(" reserve:").append(new String(vReserve));
            }
            return buf.toString();
        } catch (ScriptException e) {
    ...
    public void setLockHeight(long nUnlockHeight) {
        this.nUnlockHeight = nUnlockHeight;
    }

    public void setReserve(byte[] vReserve) {
        this.vReserve = vReserve;
    }

    public SafeReserve getSafeReserve() {
        if (mSafeReserve == null) {
            mSafeReserve = SafeUtils.parseReserve(vReserve);
        }
        return mSafeReserve;
    }

    public boolean isSPOSLockTx() {
        return true;
    }

    public boolean isDashTx() { //是否Dash交易
        return getSafeReserve().isDashTx();
    }

    public boolean isSafeTx() { //是否safe交易
        return getSafeReserve().isSafeTx();
    }

    public String getCurrentAssetId() {
        SafeReserve reserve = getSafeReserve();
        if (reserve.isIssue()) {
            return SafeUtils.generateAssetId(reserve.getIssueProtos());

        } else if (reserve.isAddIssue() || reserve.isTransfer() || reserve.isAssetChange()) {
            SafeProtos.CommonData protos = reserve.getCommonProtos();
            return SafeUtils.hash256ToAssetId(protos.getAssetId().toByteArray());
        } else if (reserve.isPutCandy()) {
            SafeProtos.PutCandyData protos = reserve.getPutCandyProtos();
            return SafeUtils.hash256ToAssetId(protos.getAssetId().toByteArray());

        } else if (reserve.isGetCandy()) {
            SafeProtos.GetCandyData protos = reserve.getGetCandyProtos();
            return SafeUtils.hash256ToAssetId(protos.getAssetId().toByteArray());

        }
        return SafeConstant.SAFE_FLAG;
    }

    public boolean isCurrentAssetTx(String assetId) {
        SafeReserve reserve = getSafeReserve();
        if (reserve.isIssue()) {
            String txAssetId = SafeUtils.generateAssetId(reserve.getIssueProtos());
            if (assetId.equals(txAssetId)) {
                return true;
            }
        } else if (reserve.isAddIssue() || reserve.isTransfer() || reserve.isAssetChange()) {
            SafeProtos.CommonData protos = reserve.getCommonProtos();
            String txAssetId = SafeUtils.hash256ToAssetId(protos.getAssetId().toByteArray());
            if (assetId.equals(txAssetId)) {
                return true;
            }
        } else if (reserve.isPutCandy()) {
