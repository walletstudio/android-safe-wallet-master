...
public abstract class AbstractBlockChain {
    ...
    public boolean add(Block block) throws VerificationException, PrunedException {
        try {
            ...
            log.info("----------- block store exception ---------");
            return false;
    ...
    private boolean add(Block block, boolean tryConnecting,
        ...
        try {
            ...
            try {
                ...
                //SPOS时间间隔检查
                if (height > Constants.NETWORK_PARAMETERS.getStartSposHeight()) {
                    if(block.getTimeSeconds() - storedPrev.getHeader().getTimeSeconds() < 28){
                        return false;
                    }
                }

                SafeUtils.handleBlock(block, storedPrev, height);
...
//            checkDifficultyTransitions(storedPrev, block);
    ...
    private StoredBlock getAutoCheckpoint(StoredBlock saveBestBlock) throws BlockStoreException {
        StoredBlock prevBlock = saveBestBlock.getPrev(blockStore);
        while (prevBlock != null && prevBlock.getHeight() + SafeConstant.BEFORE_CHECK_POINT - 1 > saveBestBlock.getHeight()) {
    ...
    //SPOS自动检查点
    private boolean checkPoint(StoredBlock saveBestBlock) throws BlockStoreException {
        StoredBlock prevBlock = getAutoCheckpoint(saveBestBlock);
        if (saveBestBlock.getHeight() <= prevBlock.getHeight())
            return false;
        return true;
    }
    ...
    private void connectBlock(final Block block, StoredBlock storedPrev, boolean expensiveChecks,
        ...
        if (storedPrev.getHeight() + 1 > Constants.NETWORK_PARAMETERS.getStartSposHeight()) {
            if (!checkPoint(storedPrev)) { //SPOS自动检查点
                throw new VerificationException("SPOS Block failed checkpoint lockin at " + (storedPrev.getHeight() + 1));
            }
        }
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        if (params.getId().equals(NetworkParameters.ID_TESTNET)) {
            if (storedPrev.getHeight() + 1 >= 3000) {
                DiffMode = 4;
            }
        } else {
            if (storedPrev.getHeight() + 1 >= 68589) {
                DiffMode = 4;
            } else if (storedPrev.getHeight() + 1 >= 34140) {
                DiffMode = 3;
            } else if (storedPrev.getHeight() + 1 >= 15200) {
                DiffMode = 2;
            } else if (storedPrev.getHeight() + 1 >= 807085 && storedPrev.getHeight() + 1 < 807185) {
                DiffMode = 1;
            } else if (storedPrev.getHeight() + 1 >= 807185 && storedPrev.getHeight() + 1 < 807285) {
                DiffMode = 2;
            } else if (storedPrev.getHeight() + 1 >= 807285) {
                DiffMode = 4;
            }
        }
        ...
        if (DiffMode == 1) {
            ...
            DarkGravityWave(storedPrev, nextBlock);
            return;
        } else if (DiffMode == 4) {
...
//            return;
        ...
        // TODO: 2018/1/30 zhangmiao update
    ...
    private void verifyDifficulty(BigInteger calcDiff, StoredBlock storedPrev, Block nextBlock) {
        ...
        if (params.getId().compareTo(params.ID_TESTNET) == 0) {
            ...
            if (height <= 68589) {
                ...
                // TODO: 2018/3/16 测试环境不判断，生产才判断
//                if(java.lang.Math.abs(n1-n2) > n1 * 0.2)
//                              throw new VerificationException("Network provided difficulty bits do not match what was calculated: " +
//                                receivedDifficulty.toString(16) + " vs " + calcDiff.toString(16));

            } else {
                // TODO: 2018/2/1 zhangmiao update
//                    if (calcDiff.compareTo(receivedDifficulty) != 0)
//                        throw new VerificationException("Network provided difficulty bits do not match what was calculated: " +
//                                receivedDifficulty.toString(16) + " vs " + calcDiff.toString(16));
